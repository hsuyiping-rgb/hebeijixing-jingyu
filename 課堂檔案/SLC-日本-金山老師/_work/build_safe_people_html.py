...
SLIDES = ROOT / "圖片" / "output" / "safe_people_html_slides"
OUT = ROOT / "HTML簡報" / "output" / "notebooklm_lesson_study_illustrated_pdf_3editable_safe_people_integrated.html"
...
def webp_data(path: Path) -> str:
    ...
    buf = BytesIO()
    im.save(buf, format="WEBP", quality=84, method=6)
    return base64.b64encode(buf.getvalue()).decode("ascii")


slides = sorted(SLIDES.glob("slide_*.png"))
if not slides:
    raise SystemExit(f"No slides found: {SLIDES}")
