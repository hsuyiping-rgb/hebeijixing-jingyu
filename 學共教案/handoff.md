...
1. **五上國語〈蚊帳大使〉依最新規範重產與補齊**：
   - 補齊 `教材/五年級_國語_蚊帳大使/教材摘要.md`（四段標準格式：單元結構表、迷思與難點、已給技術步驟、頁碼對照）。
   - 產出於 `教案成果/五年級_國語_蚊帳大使_20260915/`，依最新版面規範（新細明體 12pt、理據表 12 項完整、表頭移除學校名稱前綴、文本結構圖 PIL 自動繪製、理答預想與串連進程模擬完整合併）。
   - 通過 Step 3 全部 15 條硬檢核。
   - `build_docx.py` 成功編譯出版控版 `教案.docx` 與含課文附件版 `教案（含課文附件）.docx`。
   - 課文附件（`附件_課文.json`、`附件_課文.md`、`教案（含課文附件）.docx`）正確被 `.gitignore` 擋下。

## 🚦 目前狀態

- 技能可用。硬檢核 15 條全部通過。
- **教案均留有 Step 0 欄位標記**（學校、班級人數、日期節次、教學者、備課成員、特殊學生），文中明確標註 `〔待補〕`。
- 熱對物質的影響另有四項待教師確認：節次是否為活動 1 第 3 節、前一節是否已做完 p.49 加熱銅球、
  pe-Ⅲ-2 全文、**銅環同時加熱 3 分鐘是否足夠（推估值，要課前試做）**。

## ➡️ 下一步

1. 等使用者提供 Step 0 資料，補教案基本欄位、觀課焦點指名對象、串連進程的生1／生2／生3，重跑 `build_docx.py`（含圖版與無圖版各一次）。
...
- **`.git/packed-refs.lock` 會反覆殘留在母 repo**（`和北極星境遇/.git/`，疑為 GDrive 同步搶檔）。commit 本身成功；
  下次 git 動作前 `rm -f` 掉即可。
- **母 repo `hebeijixing-jingyu` 是 PUBLIC。** 不進版控：`範本/`、`教材/`、課文附件、`附件_課文*`、`附件_課本*`、`*（含課本圖）.docx`、`*（含課本附件）.docx`、`tmp/`。
...
- 時間：2026-09-15
- 更新者：**Antigravity (Gemini 3.8 Flash)** @ DESKTOP-31QBU95
- Git 狀態：新產出於 `教案成果/五年級_國語_蚊帳大使_20260915/`，待使用者指令 commit
