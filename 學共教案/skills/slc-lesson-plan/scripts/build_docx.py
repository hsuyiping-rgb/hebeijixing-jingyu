...
範本的已知結構（勿假設，改版時請先用 --inspect 重新確認）：
    ...
    T[2] 3x3  領域課程綱要 ⚠️ **r2 的第 2、3 欄是合併儲存格**，整列只有 2 個實際格
...
def build(content, template, out):
    ...
    # 領域課程綱要
    # ⚠️ r2 的第 2、3 欄是合併儲存格：學習內容與學習目標必須並存於同一格。
    g = c['領域課程綱要']
    for i, hd in enumerate(['學習表現', '學習內容', '學習目標']):
        f.cell(T[2].rows[1].cells[i], hd, bold=True)
    f.cell(T[2].rows[2].cells[0], g['學習表現'])
    if Filler.distinct(T[2].rows[2]) < 3:
        merged = ['【學習內容】' + g['學習內容'], ''] + ['【學習目標】'] + list(g['學習目標'])
        f.cell(T[2].rows[2].cells[1], merged)
    else:
        f.cell(T[2].rows[2].cells[1], g['學習內容'])
        f.cell(T[2].rows[2].cells[2], list(g['學習目標']))
